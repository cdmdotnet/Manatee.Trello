...
namespace Manatee.Trello
{
	...
	{
		private static readonly OneToOneMap<CheckItemState, string> _stateMap;

		private IJsonCheckItem _jsonCheckItem;
		private Position _position;
		private CheckItemState _state = CheckItemState.Unknown;
		...
		public CheckItemState State
		{
			get
			{
				if (_isDeleted) return CheckItemState.Unknown;
				VerifyNotExpired();
				return _state;
			}
			set
		...
		static CheckItem()
		{
			_stateMap = new OneToOneMap<CheckItemState, string>
						{
		...
		{
			_state = _stateMap.Any(kvp => kvp.Value == _jsonCheckItem.State) ? _stateMap[_jsonCheckItem.State] : CheckItemState.Unknown;
		}
